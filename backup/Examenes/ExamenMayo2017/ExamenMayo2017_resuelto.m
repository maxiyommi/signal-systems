...
% Funcion que resuelve el ejercicio 1
    ...
    alumnos = (1:length(notas));
    promedio(alumnos) = (notas(alumnos,2)+notas(alumnos,3)+notas(alumnos,4))/3;
    promedio = promedio';
    notas = [notas promedio];

    notas = sortrows(notas,5);
    notas_aprobados = notas(find(notas(:,5)>=60,1):end,:);
    notas_desaprobados = notas(1:(find(notas(:,5)>=60,1))-1,:);
    
    save -ascii notas_aprobados.dat notas_aprobados 
    save -ascii notas_desaprobados.dat notas_desaprobados 
    
end

function ejercicio2 ()
% Funcion que resuelve el ejercicio 2
